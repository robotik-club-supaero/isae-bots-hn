...
from .test_config import TEST_ACTIONS
...
class ActionTestNode(Node):
    ...
    def __init__(self):
        ...
        # Publishers
        self.pub_cursor     = self.create_publisher(Int16, '/act/cursor',          default_profile)
        self.pub_drawbridge = self.create_publisher(Int16, '/act/order/drawbridge', default_profile)
    ...
    def _send_next_action(self):
        ...
        actuator, order = self.current_action
        self.waiting = True
        ...
        if actuator == 'cursor':
            self.get_logger().info(f"→ cursor order: {order}")
            self.pub_cursor.publish(msg)
        elif actuator == 'drawbridge':
            self.get_logger().info(f"→ drawbridge order: {order}")
    ...
    def _on_cursor_callback(self, msg):
        ...
        if msg.data == order:
            ...
            self.get_logger().warning(f"Unexpected cursor callback: {msg.data} (expected {order})")
    ...
    def _on_drawbridge_callback(self, msg):
        ...
        if msg.data == order:
            ...
            self.get_logger().warning(f"Unexpected drawbridge callback: {msg.data} (expected {order})")
