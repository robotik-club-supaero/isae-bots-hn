...
#################################################################
# TakeCherries - For cherries (perpendicular or not)
#################################################################

class TakeCherriesPerpendicular(smach.State) :
    def __init__(self) :
            smach.State.__init__(   self,
                                    outcomes=['fail', 'preempted','done'],
                                    input_keys=['nb_actions_done','cb_arm', 'nb_take_cherries_error'],
                                    output_keys=['nb_actions_done', 'nb_take_cherries_error'])
    ...
    def execute(self, userdata):
        ...
        userdata.cb_arm[0] = -1
        cherries_pub.publish(1) # Publish 1 to tell to the BN to deploy the arm
