...
>Attention, ceci est le tutoriel pour PC. Si vous voulez configurer le Raspberry, consultez le tutoriel "4. Configurer le Raspberry".
...
echo \
  ...
  $(. /etc/os-release && echo "${UBUNTU_CODENAME:-$VERSION_CODENAME}") bookworm stable" | \
  sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
# NOUVELLE VERSION DE LA LIGNE PRECEDENTE
echo \
  "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/ubuntu \
...
>Si vous ne l'avez pas fait à l'étape précédente, fermez la session et reconnectez-vous (ou redémarrez WSL).
