...
- Enfin, copiez le nouveau fichier `10-usb-serial.rules` dans le conteneur Docker du Raspberry, à l'emplacement indiqué : `/etc/udev/rules.d/10-usb-serial.rules`. Pour que les modifications prennent effet, faites la commande `sudo udevadm control --reload-rules` ou redémarrez le Raspberry.
