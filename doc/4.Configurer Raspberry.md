...
# Annexe 6 - Accès aux ports série depuis Python

TODO: trouver une meilleure solution !

Si MicroRos n'arrive pas à communiquer avec la BR ou les actionneurs lorsqu'il est lancé depuis le `master_node`, il faut modifier les permissions des ports. Par exemple mettre dans `/etc/rc.local` :

```bash
#!/bin/sh -e
chown root:root /dev/ttyACT
chown root:root /dev/ttyBR
