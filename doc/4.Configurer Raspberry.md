...
## Mise en garde - ROS2 via le réseau

Si votre PC est connecté au même réseau que le Raspberry, les messages ROS2 que vous envoyez depuis votre PC sont communiqués au robot (et réciproquement) ! Cela vous permet par exemple d'ouvrir l'interface sur votre PC pour vérifier que le robot est bien là où il croit être et que les obstacles sont détéctés correctement.

En revanche, cela peut également provoquer des conflits si vous voulez envoyez des messages ROS qui ne sont pas destinés au robot.
...
- Enfin, copiez le nouveau fichier `10-usb-serial.rules` sur le Raspberry, à l'emplacement indiqué : `/etc/udev/rules.d/10-usb-serial.rules`. Pour que les modifications prennent effet, faites les commandes `sudo udevadm control --reload-rules` et `sudo udevadm trigger` ou redémarrez le Raspberry.
...
# Annexe 6 - Démarrage automatique du `master_node` ou du `remote_control` (manette Wii)

Pour cela, il faut créer un "service" Linux. Le dossier `service` contient les services suivants :
- `masterNode.service` pour démarrer automatiquement le master_node
- `remoteControl.service` pour démarrer automatiquement le contrôle à distance avec la manette Wii
- `match.service` pour démarrer automatiquement la stratégie (si vous ne voulez pas utiliser le master_node)

## Installer un service
Pour installer un service, il faut le copier dans `/etc/systemd/system/` (sur le Raspberry, en dehors du Docker).

Example :
```
cd isae-bots-hn-2024
sudo cp service/masterNode.service /etc/systemd/system/masterNode.service
```

> Attention, ce dossier contient tous les services de toutes les applications installées, pas seulement les nôtres !

## Activer un service
Une fois installé, le service doit être activé :
...
Vous pouvez installer plusieurs services mais un seul des trois doit être activé à la fois, sinon vous aurez des conflits.

## Désactiver un service
```
sudo systemctl stop masterNode
sudo systemctl disable masterNode
```

## Attention

Le service démarre le Docker. Si le Docker s'arrête, le service aussi et vous devez le redémarrer manuellement (ou redémarrer le Raspberry).
Pour éviter cela, utilisez `make term` (au lieu de `make main`) pour aller dans le Docker quand un service est activé.